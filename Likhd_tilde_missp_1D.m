function [ nelogL ] = Likhd_tilde_missp_1D( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_est,opt )
% Negative log-likelihood function with the misspecified missing data model and
% the grouped outcome $\tilde{Y}^star$, i.e. the setting at (2.2) in the paper.
% Input:
% X: N*1 1-d covariate;
...
parfor j = 1:length(Y_star_tilde)
    ...
    for i = sum(n1_par(1:j))+1:sum(n1_par(1:j+1))
        if RX_par(i) == 1 
            f_z = f_z + log(normpdf(X_1_par(i),t_x(1),t_x(2))) + log(f_r_mis_1D(X_1_par(i),0,1,1,t_r)) + log(1-p(X_1_par(i)));% log(f_W_D_0)
            f_o = f_o + log(normpdf(X_1_par(i),t_x(1),t_x(2))) + log(f_r_mis_1D(X_1_par(i),1,1,1,t_r).*p(X_1_par(i)) + f_r_mis_1D(X_1_par(i),0,1,1,t_r).*(1-p(X_1_par(i))) );% log(f_W_D_0-f_W_D_1)
        else
            inti_z = @(x) normpdf(x,t_x(1),t_x(2)).*(1-p(x)).*f_r_mis_1D(x,0,0,1,t_r);
            f_z = f_z + log(integral(inti_z,-Inf,Inf));% log(\int f_W_D_0 dx)
            inti_o = @(x) normpdf(x,t_x(1),t_x(2)).*(f_r_mis_1D(x,1,0,1,t_r).*p(x) + f_r_mis_1D(x,0,0,1,t_r).*(1-p(x)) );
...
for i = (N_obs+1):length(RD)
    ...
    if RX(i) == 1
        res = res + log(normpdf(X(i),t_x(1),t_x(2)).* ( p(X(i)).*f_r_mis_1D(X(i),1,1,0,t_r)+(1-p(X(i))).*f_r_mis_1D(X(i),0,1,0,t_r) )); 
    else
        inti_one = @(x) normpdf(x,t_x(1),t_x(2)).* ( p(x).*f_r_mis_1D(x,1,0,0,t_r)+(1-p(x)).*f_r_mis_1D(x,0,0,0,t_r) );
