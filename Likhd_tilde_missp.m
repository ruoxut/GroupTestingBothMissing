function [ nelogL ] = Likhd_tilde_missp( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_fix,t_est,opt )
% Negative log-likelihood function with misspecified missing data model
% and the grouped outcome $\tilde{Y}^star$, i.e. the setting at (2.2) in the paper.
% Input:
% X: N*2 2-d covariate;
...
% Parameters for f_X and f_r_mis
...
parfor j = 1:length(Y_star_tilde)
    ...
    for i = sum(n1_par(1:j))+1:sum(n1_par(1:j+1))
        if RX_par(i) == 1 
            f_z = f_z + log(f_X(X_1_par(i),X_2_par(i),t_x)) + log(f_r_mis(X_1_par(i),X_2_par(i),0,1,1,t_r)) + log(1-p(X_1_par(i),X_2_par(i)));% log(f_W_D_0)
            f_o = f_o + log(f_X(X_1_par(i),X_2_par(i),t_x)) + log(f_r_mis(X_1_par(i),X_2_par(i),1,1,1,t_r).*p(X_1_par(i),X_2_par(i)) + f_r_mis(X_1_par(i),X_2_par(i),0,1,1,t_r).*(1-p(X_1_par(i),X_2_par(i))) );% log(f_W_D_0-f_W_D_1)
        else
            inti_z = @(x) f_X(X_1_par(i),x,t_x).*(1-p(X_1_par(i),x)).*f_r_mis(X_1_par(i),x,0,0,1,t_r);
            f_z = f_z + log(integral(inti_z,-Inf,Inf));% log(\int f_W_D_0 dx)
            inti_o = @(x) f_X(X_1_par(i),x,t_x).*(f_r_mis(X_1_par(i),x,1,0,1,t_r).*p(X_1_par(i),x) + f_r_mis(X_1_par(i),x,0,0,1,t_r).*(1-p(X_1_par(i),x)) );
...
for i = (N_obs+1):length(RD)
    ...
    if RX(i) == 1
        res = res + log(f_X(X(i,1),X(i,2),t_x).* ( p(X(i,1),X(i,2)).*f_r_mis(X(i,1),X(i,2),1,1,0,t_r)+(1-p(X(i,1),X(i,2))).*f_r_mis(X(i,1),X(i,2),0,1,0,t_r) )); 
    else
        inti_one = @(x) f_X(X(i,1),x,t_x).* ( p(X(i,1),x).*f_r_mis(X(i,1),x,1,0,0,t_r)+(1-p(X(i,1),x)).*f_r_mis(X(i,1),x,0,0,0,t_r) );
