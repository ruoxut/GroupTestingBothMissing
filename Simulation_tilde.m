...
% This is to reproduce the simulation results of models (iii) and (iv) for the estimators
...
for s = 1:3
    ...
    for n_rep_ind = 1:n_rep
        ...
        % Reordering data
        n1 = [0;n];
        data_1 = [X(RD==1,:) D(RD==1) RX(RD==1) RD(RD==1)];
        data_0 = [X(RD==0,:) D(RD==0) RX(RD==0) RD(RD==0)];
        ...
        % $\hat{p}_{MLE,1}^{LOG}$ 
        nelogL = @(t_est) Likhd_tilde( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_fix,t_est,opt );  
        ...
        % $\hat{p}_{MLE,1}^{MISP}$
        nelogL = @(t_est) Likhd_tilde_missp( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_fix,t_est,opt ); 
        t_est_missp = fminunc(nelogL,t_ini,options);
        Opt_res_missp{n_rep_ind} = t_est_missp;% Estimated parameters
        if opt == 1
            p_hat_missp = 1./(8+exp(t_est_missp(4).*x_1_eva+t_est_missp(5).*x_2_eva+t_est_missp(6)));
        elseif opt == 2
            p_hat_missp = 1./(1+exp(t_est_missp(4).*x_1_eva.^2+t_est_missp(5).*x_2_eva+t_est_missp(6)));
        end
        sqer_missp = (p_hat_missp-p_eva).^2;
        ISE_missp(n_rep_ind) = trapz(x_2_range,trapz(x_1_range,sqer_missp));
        
        % $\hat{p}_{MLE,1}^{IGN}$
        nelogL = @(t_est) Likhd_tilde_ign( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_fix,t_est,opt );  
        t_est_ign = fminunc(nelogL,t_ini_naive,options);
        Opt_res_ign{n_rep_ind} = t_est_ign;% Estimated parameters
        if opt == 1
            p_hat_ign = 1./(8+exp(t_est_ign(4).*x_1_eva+t_est_ign(5).*x_2_eva+t_est_ign(6)));
        elseif opt == 2
            p_hat_ign = 1./(1+exp(t_est_ign(4).*x_1_eva.^2+t_est_ign(5).*x_2_eva+t_est_ign(6)));
        end
    ...
    % Store results
    fname = sprintf('211123_XY_(%d)_A_tilde_J%d',opt,J_sample(s));
