function [ nelogL ] = Likhd_tilde_ign_v( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_est )
% Negative log-likelihood function ignoring missing data mechanism 
% with the grouped outcome $\tilde{Y}^star$, i.e. the setting at (2.2) for model (v) in the paper.
% Input:
% X: N*2 2-d covariate;
...
parfor j = 1:length(Y_star_tilde) 
   ...
   for i = sum(n1(1:j))+1:sum(n1(1:j+1))
       ...
       elseif RX(i)==1 && RD(i)==0
           ...
           f_1 = f_1 + log(f_X(X(i,1),X(i,2),t_x));% log(f_W_D_0-f_W_D_1)
       end
   end
