...
for s = 1:3
    ...
    for n_rep_ind = 1:n_rep
        ...
        X = [data_1(:,1);data_0(:,1)];
        D = [data_1(:,2);data_0(:,2)];
        RX = [data_1(:,3);data_0(:,3)];
        RD = [data_1(:,4);data_0(:,4)];
        ...
        % $\hat{p}_{MLE,1}^{LOG}$
        nelogL = @(t_est) Likhd_tilde_1D( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_est,opt );  
        ...
        % $\hat{p}_{MLE,1}^{MISP}$
        nelogL = @(t_est) Likhd_tilde_missp_1D( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_est,opt ); 
        ...
        % $\hat{p}_{MLE,1}^{IGN}$
        nelogL = @(t_est) Likhd_tilde_ign_1D( X,Y_star_tilde,RX,RD,n,s_1,s_2,t_est,opt );  
    ...
    % Store results
    fname = sprintf('220728_XY_1D_(%d)_B_tilde_J%d',opt,J_sample(s));
