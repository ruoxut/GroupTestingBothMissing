...
for s = 1:3
    ...
    for n_rep_ind = 1:n_rep
        ...
        %  $\hat{p}_{MLE,2}^{MISP}$
        nelogL = @(t_est) Likhd_missp( X,Y_star,RX,RD,n,s_1,s_2,t_fix,t_est,opt );
        ...
        % $\hat{p}_{MLE,2}^{IGN}$
        nelogL = @(t_est) Likhd_ign( X,Y_star,RX,RD,n,s_1,s_2,t_fix,t_est,opt );
    ...
    % Store results
    fname = sprintf('211104_XY_(%d)_A_J%d',opt,J_sample(s));
